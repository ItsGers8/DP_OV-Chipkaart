...
WHERE u.cursus = c.code AND m.mnr = u.docent;
...
SELECT cursist.naam cursist, docent.naam docent FROM medewerkers cursist, medewerkers docent,inschrijvingen i,uitvoeringen u
WHERE i.cursus = 'S02' AND u.cursus = 'S02' AND cursist.mnr = i.cursist
AND docent.mnr = u.docent AND i.begindatum = u.begindatum;

-- S3.3.
-- Geef elke afdeling (`afdeling`) met de naam van het hoofd van die
-- afdeling (`hoofd`).
DROP VIEW IF EXISTS s3_3; CREATE OR REPLACE VIEW s3_3 AS                                                     -- [TEST]
SELECT a.naam afdeling, m.naam hoofd FROM afdelingen a, medewerkers m WHERE m.mnr = a.hoofd;

-- S3.4.
-- Geef de namen van alle medewerkers, de naam van hun afdeling (`afdeling`)
-- en de bijbehorende locatie.
DROP VIEW IF EXISTS s3_4; CREATE OR REPLACE VIEW s3_4 AS                                                     -- [TEST]
SELECT m.naam naam, a.naam afdeling, a.locatie FROM medewerkers m, afdelingen a WHERE m.afd = a.anr;

-- S3.5.
-- Geef de namen van alle cursisten die staan ingeschreven voor de cursus S02 van 12 april 2019
DROP VIEW IF EXISTS s3_5; CREATE OR REPLACE VIEW s3_5 AS                                                     -- [TEST]
SELECT m.naam naam FROM medewerkers m, inschrijvingen i WHERE i.cursist = m.mnr AND i.cursus = 'S02' AND i.begindatum = '2019-04-12';

-- S3.6.
-- Geef de namen van alle medewerkers en hun toelage.
DROP VIEW IF EXISTS s3_6; CREATE OR REPLACE VIEW s3_6 AS                                                     -- [TEST]
SELECT m.naam naam, s.toelage toelage FROM medewerkers m, schalen s WHERE m.maandsal > s.ondergrens AND m.maandsal <= s.bovengrens;
